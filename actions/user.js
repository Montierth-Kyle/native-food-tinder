...
export const createUser = (email, password, firstName, lastName, passwordConfirmation, title, history) => {
  return (dispatch) => {
    ...
    fetch(`/api/auth/${endpoint}`, {
      ...
      credentials: 'include',
      method: 'POST',
      body: JSON.stringify({ email, password, firstName, lastName, passwordConfirmation })
   }).then( res => res.json() )
     .then( user => {
